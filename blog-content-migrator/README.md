...
> Move posts, pages, or any custom post type — with images, internal links and ACF custom fields — from one WordPress site straight into another. Pick a whole post type, or paste a handful of URLs and migrate just those.
...
- **Content & meta** — title, slug, content, excerpt, status, date, categories/tags, and every custom field (postmeta) are copied, including SEO plugin fields (RankMath, Yoast, etc.) since those are just postmeta too.
...
- **Missing post types are auto-registered** on the target site (with a menu, so imported content is actually visible), and **the matching ACF field group is recreated automatically** if ACF is active on the target — using ACF's own import mechanism, so fields display and edit exactly as they did on the source.
...
**Storage** — imported posts get two meta keys: `_bcm_source_id` (the post's ID on the source site, used for the re-run skip check) and `_bcm_source_url` (its original permalink). Auto-registered post types are persisted in the `bcm_dynamic_post_types` option and re-registered on every `init`.
...
- **No UI for re-mapping categories/tags** to existing target-site terms — same-named terms are created or reused by name only.
